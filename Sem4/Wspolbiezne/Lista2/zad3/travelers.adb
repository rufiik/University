...
procedure Travelers is
   ...
   TrapDelay       : constant Duration := 0.1;

   Board_Width     : constant Integer := 15;
   Board_Height    : constant Integer := 15;

   Nr_Of_Traps     : constant Integer := 60;

   Start_Time      : Time := Clock;
   Seeds           : Seed_Array_Type (1 .. Nr_Of_All + Nr_Of_Traps) := Make_Seeds (Nr_Of_All + Nr_Of_Traps);

protected Sync is
   procedure Traveler_Finished;
   entry Wait_Until_All_Done;
private
   Count : Natural := 0;
end Sync;

protected body Sync is
   procedure Traveler_Finished is
   begin
      Count := Count + 1;
   end Traveler_Finished;

   entry Wait_Until_All_Done when Count = Nr_Of_All is
   begin
      null;
   end Wait_Until_All_Done;
end Sync;
 ...
 function Positions_Equal(P1, P2 : Position_Type) return Boolean is
   begin
 ...
 type Trace_Type is record
  ...
  type Trace_Array_type is array(0 .. Max_Steps * 2) of Trace_Type;
   ...
   type Trap_Array_Type is array (1 .. Nr_Of_Traps) of Position_Type;
   Trap_Trace : Traces_Sequence_Type;
  ...
  task body Printer is
   begin
      for I in 1 .. Nr_Of_All+Nr_Of_Traps  loop
    ...
    type Trap_Record is record
      Position : Position_Type;
      Traces   : Traces_Sequence_Type;
      Id       : Integer;
   end record;
    Traps : array (1 .. Nr_Of_Traps) of Trap_Record;

 procedure Store_Trap_Trace(T : in out Trap_Record) is
      Time_Stamp : Duration := To_Duration (Clock - Start_Time);
   begin
      T.Traces.Last := T.Traces.Last + 1;
      T.Traces.Trace_Array(T.Traces.Last) := (
         Time_Stamp => Time_Stamp,
         Id         => T.Id,
         Position   => T.Position,
         Symbol     => '#'
      );
   end Store_Trap_Trace;

   function Is_Trap(Position : Position_Type) return Boolean is
   begin
      for T of Traps loop
         if T.Position = Position then
            return True;
         end if;
      end loop;
      return False;
            ...
            Traveler.Position :=
              (X => Integer (Float'Floor (Float (Board_Width) * Random (G))),
               Y => Integer (Float'Floor (Float (Board_Height) * Random (G))));
                  if not Is_Trap(Traveler.Position) then
               select
      ...
      exit;
         ...
         if Deadlock then
            ...
            Traveler.Position := New_Position;
              if Is_Trap(Traveler.Position) then
               Traveler.Symbol := Character'Val(Character'Pos(Traveler.Symbol) + 32); 
               Time_Stamp := To_Duration (Clock - Start_Time);
               Store_Trace;
               delay TrapDelay;
               ...
               for T of Traps loop
                  if Positions_Equal(T.Position, Traveler.Position) then
                     Store_Trap_Trace(T);
                     exit;
                  end if;
               end loop;
   ...
   begin
      accept Init (Id : Integer; Seed : Integer; Symbol : Character) do
         ...
         Time_End := 25 * Max_Delay * Duration (Random (G));
      end Init;
   ...
   Symbol : Character;
   Id     : Integer;
procedure Initialize_Traps is
      G : Generator;
      Pos : Position_Type;
      Conflict : Boolean;
      Temp_Trace : Traces_Sequence_Type;
   begin
      Reset (G, Seeds(Nr_Of_All + 1));
      for I in 1 .. Nr_Of_Traps loop
         ...
         Traps(I).Position := Pos;
         Traps(I).Id := Nr_Of_All + I-1;
         Traps(I).Traces.Last := -1;
         Store_Trap_Trace(Traps(I));
      end loop;
   end Initialize_Traps;
...
procedure Report_Traps is
begin
   for T of Traps loop
      if T.Traces.Last >= 0 then
         Printer.Report(T.Traces);
      end if;
   end loop;
end Report_Traps;
begin
   ...
   Put_Line(
      "-1 "
      & Integer'Image(Nr_Of_All+Nr_Of_Traps)
      & " "
