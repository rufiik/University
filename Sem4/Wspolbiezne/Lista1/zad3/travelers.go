...
case <-time.After(time.Duration(Max_Delay * float64(time.Second))):
    ...
    func (t *Traveler_Type) Init(id int, symbol rune, reportChan chan Traces_Sequence_Type) {
        ...
        t.Symbol = symbol
        for {
            InitMutex.Lock() // Zablokuj globalny mutex
            t.Position = Position_Type{
                X: rand.Intn(Board_Width),
                Y: rand.Intn(Board_Height),
            }
            if Board[t.Position.X][t.Position.Y].TryLock() {
                InitMutex.Unlock() // Zwolnij globalny mutex
                break
            }
            InitMutex.Unlock() // Zwolnij globalny mutex, jeśli pole jest zajęte
        }
        t.Active= true 
...
func (t *Traveler_Type) Start() {
    for step := 0; step < t.Nr_of_Steps; step++ {
        if !t.Active {
            break // Przerwij pętlę, jeśli podróżnik nie jest aktywny
        }
