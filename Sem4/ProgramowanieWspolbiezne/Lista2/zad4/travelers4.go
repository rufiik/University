...
const (
	...
	NrOfWildTenants   = 10
	NrOfTraps         = 60
	NrOfAll           = NrOfTravelers + NrOfWildTenants + NrOfTraps
	MinSteps          = 10
	MaxSteps          = 100
	MinDelay          = 0.01
	MaxDelay          = 0.05
...
var (
	wg        sync.WaitGroup
	startTime = time.Now()
	trapIDs    = make(map[Position]int)
)

func fieldServer(x, y int, isTrap bool, trapID int, requestChan chan FieldRequest) {
...
func initBoard(reportChannel chan TraceSequence) [][]chan FieldRequest {
	board := make([][]chan FieldRequest, BoardWidth)
	trapID := NrOfTravelers + NrOfWildTenants + 1
	traps := make(map[Position]int)
	for len(traps) < NrOfTraps {
		x := rand.Intn(BoardWidth)
		y := rand.Intn(BoardHeight)
		pos := Position{x, y}
		if _, exists := traps[pos]; !exists {
			traps[pos] = trapID
			trapID++
		}
	}
	...
	for i := range board {
		...
		for j := range board[i] {
			...
			id := NrOfAll+i
			pos := Position{i, j}
			if tid, exists := traps[pos]; exists {
				isTrap = true
				id = tid
			}
			board[i][j] = make(chan FieldRequest)
			go fieldServer(i, j, isTrap, id, board[i][j])
		}
	}

	for pos, id := range traps {
		trapTrace := Trace{
			Timestamp: 0,
			ID:        id,
			Position:  pos,
		...
		reportChannel <- TraceSequence{Traces: []Trace{trapTrace}}
	}
...
func travelerRoutine(t *Traveler, reportChannel chan TraceSequence, board [][]chan FieldRequest) {
	...
	for {
		...
		resp := make(chan interface{})
		board[x][y] <- FieldRequest{Type: CheckTrap, Response: resp}
		isTrap := (<-resp).(bool)
		if isTrap {
			continue
		}
	...
	for i := 0; i < t.Steps && !t.Dead; i++ {
		...
		for attempts := 0; attempts < 4; attempts++ {
			...
			if t.tryMove(newPos, board) {
				checkTrapResp := make(chan interface{})
				board[newPos.X][newPos.Y] <- FieldRequest{Type: CheckTrap, Response: checkTrapResp}
				if (<-checkTrapResp).(bool) {
					getTrapIDResp := make(chan interface{})
					board[newPos.X][newPos.Y] <- FieldRequest{Type: GetTrapID, Response: getTrapIDResp}
					trapID := (<-getTrapIDResp).(int)
					...
					time.Sleep(time.Duration(TrapActivationDur *float64(time.Second)))

					leaveResp := make(chan interface{})
					board[newPos.X][newPos.Y] <- FieldRequest{Type: Leave, Response: leaveResp}
					<-leaveResp
	...
	for {
		...
		case <-wt.MoveOutChan:
			for _, dir := range rand.Perm(4) {
				...
				resp := make(chan interface{})
				board[possiblePos.X][possiblePos.Y] <- FieldRequest{Type: CheckOccupant, Response: resp}
				if (<-resp) == nil {
					setResp := make(chan interface{})
					...
					if (<-setResp).(bool) {
						leaveResp := make(chan interface{})
						board[wt.Position.X][wt.Position.Y] <- FieldRequest{Type: Leave, Response: leaveResp}
						<-leaveResp

						checkTrapResp := make(chan interface{})
						board[possiblePos.X][possiblePos.Y] <- FieldRequest{Type: CheckTrap, Response: checkTrapResp}
						if (<-checkTrapResp).(bool) {
							getTrapIDResp := make(chan interface{})
							board[possiblePos.X][possiblePos.Y] <- FieldRequest{Type: GetTrapID, Response: getTrapIDResp}
							trapID := (<-getTrapIDResp).(int)
							...
							leaveResp := make(chan interface{})
							board[possiblePos.X][possiblePos.Y] <- FieldRequest{Type: Leave, Response: leaveResp}
							<-leaveResp
...
func main() {
    ...
    fmt.Printf("-1  %d  %d  %d\n", NrOfAll+NrOfTraps, BoardWidth, BoardHeight)

    reportChannel := make(chan TraceSequence, NrOfAll*MaxSteps)
	board := initBoard(reportChannel)
