...
def load_results(file_path):
    ...
    return data[:, 0], data[:, 1], data[:, 2], data[:, 3]  # n, comparisons (c), swaps (s)

# Funkcja do obliczania średnich wartości
def calculate_averages(n_values, comparisons, swaps, time):
    ...
    for n in unique_n:
        ...
        avg_time.append(np.mean(time[mask]))
    return unique_n, np.array(avg_comparisons), np.array(avg_swaps), np.array(avg_time)

# Ścieżki do plików wynikowych
algorithms = ["quick", "DualPivotQS", "quickold", "DualPivotQSold"]
colors = ["blue", "green", "red", "orange"]
results_dir = "./wyniki/"
output_dir = "./wykresy2/"  # Folder do zapisu wykresów
k_values = [1, 10, 100]

# Tworzenie folderu na wykresy, jeśli nie istnieje
if not os.path.exists(output_dir):
    os.makedirs(output_dir)
def estimate_constant(n_values, comparisons):
    n_log_n = n_values * np.log2(n_values)  # Oblicz n * log(n)
    constants = comparisons / n_log_n       # Wyznacz stałe dla każdego n
    return np.mean(constants)  
# Generowanie wykresów dla różnych kategorii danych
categories = ["c", "s", "c_n", "s_n", "time"]
    ...
    for ax, (k, color) in zip(axes, zip(k_values, ["blue", "green", "red"])):  # Kolory dla różnych k
        for algo, algo_color in zip(algorithms, colors):
            ...
            if os.path.exists(file_path) and os.stat(file_path).st_size > 0:
                n_values, comparisons, swaps, time = load_results(file_path)
                n_unique, avg_comparisons, avg_swaps, avg_time = calculate_averages(n_values, comparisons, swaps, time)

                # Wybór danych do rysowania
                if category == "c":
                    ax.plot(n_unique, avg_comparisons, label=f"{algo}", color=algo_color)
                    C = estimate_constant(n_unique, avg_comparisons)
                    n_log_n = n_unique * np.log2(n_unique)
                    ax.plot(n_unique, C * n_log_n, label=f"{algo} (C={C:.2f})", linestyle="--", color=algo_color)
                elif category == "s":
                    ...
                    ax.plot(n_unique, avg_time, label=f"{algo}", color=algo_color)
